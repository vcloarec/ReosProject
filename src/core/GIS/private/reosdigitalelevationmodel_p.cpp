...
ReosRasterMemory<float> *ReosDigitalElevationModelRaster::extractMemoryRasterSimplePrecision( const QRectF &destinationExtent, ReosRasterExtent &outputRasterExtent, const QString &destinationCrs )
{
  QgsCoordinateReferenceSystem destCrs = QgsCoordinateReferenceSystem::fromWkt( destinationCrs );
  QgsCoordinateTransform transform( mCrs, destCrs, mTransformContext );

  QgsRectangle destExtent( destinationExtent );
...
ReosRasterExtent ReosDigitalElevationModelRaster::rasterExtent( const QgsRectangle &originalExtent ) const
{
  ...
  {
    xPixelSize = sourceRasterExtent.width() / mDataProvider->xSize();
    yPixelSize = sourceRasterExtent.height() / mDataProvider->ySize();
  }
  else
  {
    xPixelSize = 1;
  ...
  return ReosRasterExtent( QRectF( QPointF( xmin, ymin ), QPointF( xmax, ymax ) ), xPixelSize, -yPixelSize );
}
