...
% End
...
#ifndef SIP_RUN
  static ReosApplication *
  initializationReos(int &argc, char **argv,
                     const QString &appName = QStringLiteral("Reos"));
#else

  /**
   * Initialization of a ReosApplication. Returns a pointer to the
   * ReosApplication singleton, caller take ownership
   *
   * \param argv command line arguments
   */
  ...
  if ((argv = qtgui_ArgvToC(a0, argc)) == NULL)
    ...
    return sipConvertFromType(ReosApplication::initializationReos(argc, argv),
                              sipType_ReosApplication, NULL);
  }
