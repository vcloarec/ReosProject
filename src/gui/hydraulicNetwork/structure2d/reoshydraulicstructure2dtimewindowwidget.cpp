  ...
  , mTimeWindowSettings( timeWindowSettings )
{
  ...
  ui->mStartTime->enableSpacer( ReosParameterWidget::SpacerInMiddle );
  ui->mEndTime->enableSpacer( ReosParameterWidget::SpacerInMiddle );
...
void Reoshydraulicstructure2dTimeWindowWidget::onAutomaticGroupBoxToggle()
{
  ...
  ui->mStartTime->setEnabled( !checked );
  ui->mEndTime->setEnabled( !checked );
}
...
{
  bool checked = mTimeWindowSettings->automaticallyDefined()->value();
  ui->mGroupBoxAuto->blockSignals( true );
  ui->mGroupBoxAuto->setChecked( checked );
