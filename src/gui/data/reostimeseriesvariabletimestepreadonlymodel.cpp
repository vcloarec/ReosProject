  ...
  if ( !selection.isEmpty() )
  {
    ...
    if ( withHeader )
    {
      ...
      for ( int w = 0; w < range.width(); ++w )
        headers.append( headerData( range.left() + w, Qt::Horizontal, Qt::DisplayRole ).toString() );
...
ReosTimeSeriesVariableTimeStepReadOnlyTablesView::ReosTimeSeriesVariableTimeStepReadOnlyTablesView( QWidget *parent ) : QWidget( parent )
{
  ...
  tableLayout->addSpacerItem( new QSpacerItem( 20, 0, QSizePolicy::Expanding ) );
...
{
  ReosParameterBooleanWidget *useConstantWidget = new ReosParameterBooleanWidget( useConstantTimeStepParameter, this );
  mTimeStepLayout->addWidget( useConstantWidget );
  ReosParameterDurationWidget *timeStepWidget = new ReosParameterDurationWidget( constantTimeSepParameter, this );
  mTimeStepLayout->addWidget( timeStepWidget );
  ...
  timeStepWidget->setVisible( useConstantTimeStepParameter->value() );
  ...
  connect( useConstantTimeStepParameter, &ReosParameterBoolean::valueChanged, mModel, [this, timeStepWidget, useConstantTimeStepParameter]
  {
    mModel->setIsFixedTimeStep( useConstantTimeStepParameter->value() );
    timeStepWidget->setVisible( useConstantTimeStepParameter->value() );
  } );
...
void ReosTimeSeriesVariableTimeStepReadOnlyTablesView::setSeries( QList < ReosTimeSerieVariableTimeStep *> series, const QString &unit )
{
  for ( ReosTimeSerieVariableTimeStep *serie : std::as_const( series ) )
    mModel->addTimeSerie( serie, tr( "%1 (%2)" ).arg( serie->name(), unit ) );
